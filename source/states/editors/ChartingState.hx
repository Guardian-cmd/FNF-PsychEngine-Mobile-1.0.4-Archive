...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	// ============================================================
	// MOBILE HITBOX MODE (Hitbox Mode / Iniciar canción / Large Sustain Notes)
	// ============================================================
	...
	var mobileHitbox:mobile.objects.Hitbox;
	var mobileLaneButtons:Array<mobile.objects.TouchButton> = [];
	...
	var disabledDuringRecording:Array<FlxBasic> = [];
	...
	function startRecording()
	{
		...
		var startTime:Float = (curSec >= 0 && curSec < cachedSectionTimes.length) ? cachedSectionTimes[curSec] : 0;
		Conductor.songPosition = startTime;
		loadMusic();
		setSongPlaying(true);

		mobileHitbox = new mobile.objects.Hitbox();
		add(mobileHitbox);

		mobileLaneButtons = [mobileHitbox.buttonLeft, mobileHitbox.buttonDown, mobileHitbox.buttonUp, mobileHitbox.buttonRight];

		mobileHitbox.onButtonDown.add(onHitboxDown);
		mobileHitbox.onButtonUp.add(onHitboxUp);

		var mine:Array<FlxBasic> = [hitboxModeBtn, largeSustainBtn, startSongBtn, stopSongBtn, sideBtn, mobileHitbox];
	...
	function stopRecording()
	{
		...
		{
			mobileHitbox.onButtonDown.remove(onHitboxDown);
			mobileHitbox.onButtonUp.remove(onHitboxUp);
			remove(mobileHitbox, true);
			mobileHitbox.destroy();
			mobileHitbox = null;
		}
		mobileLaneButtons = [];
	...
	function laneOfMobileButton(btn:mobile.objects.TouchButton):Int
	{
		return mobileLaneButtons.indexOf(btn);
	}

	function onHitboxDown(btn:mobile.objects.TouchButton)
	{
		if (!recordingActive)
			return;
		var lane:Int = laneOfMobileButton(btn);
		if (lane < 0)
			return;
		beginNoteAtLane(lane);
	}

	function onHitboxUp(btn:mobile.objects.TouchButton)
	{
		if (!recordingActive)
			return;
		var lane:Int = laneOfMobileButton(btn);
	...
	function sectionForRecordingTime(t:Float):Int
	{
		var sec = 0;
	...
	function beginNoteAtLane(lane:Int)
	{
		...
		var secNum:Int = sectionForRecordingTime(strumTime);
	...
	function finishNoteAtLane(lane:Int)
	{
		...
		if (note != null && largeSustainOn && FlxG.sound.music != null)
		{
			...
			if (heldMs > 50)
			{
				var secNum:Int = sectionForRecordingTime(hitboxHoldStart[lane]);
